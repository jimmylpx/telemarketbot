# 🤖 TeleMarketBot — Telegram Auto Order & QRIS Merchant Gateway
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Payment-QRIS%20All%20Payment-118EEA?style=flat-square" alt="QRIS">
...
Solusi self-hosted lengkap untuk bot Telegram jualan produk digital berbasis baris teks otomatis (seperti **link Jio**, akun streaming/login, token, voucher, cookies, lisensi). Terintegrasi langsung dengan **QRIS Dinamis (GoPay Merchant, DANA Bisnis, Bank, dsb)**, **Web Admin Panel**, dan **Instant Auto-Delivery stok**.
...
- 💳 **QRIS Dinamis Otomatis (Auto EMVCo):** Bot secara otomatis menyuntikkan nominal tagihan ke QRIS statis merchant Anda menggunakan kalkulasi checksum CRC-16 standar Bank Indonesia. Pembeli tinggal scan tanpa perlu ketik nominal manual!
- 🔔 **Auto-Check Pembayaran (MacroDroid Webhook):** Setiap ada notifikasi uang masuk (GoPay Merchant, DANA, BCA, OVO, dsb) di HP Anda, MacroDroid langsung meneruskan notifikasi ke Webhook server. Pesanan diverifikasi lunas dalam hitungan detik.
...
4. `QRIS_BASE_PAYLOAD` (hasil scan string QRIS merchant Anda)
...
## 📱 Panduan Integrasi MacroDroid (Auto-Check Pembayaran QRIS)

Agar bot dapat memverifikasi pembayaran secara otomatis saat pembeli mentransfer dana via QRIS:

1. Pasang aplikasi **[MacroDroid](https://play.google.com/store/apps/details?id=com.arlosoft.macrodroid)** di smartphone Android tempat aplikasi penerima notifikasi pembayaran login (GoPay Merchant, DANA Bisnis, BCA Mobile, dsb).
2. Buat Makro Baru:
   - **Trigger (Pemicu):**
     - Pilih `Device Events` -> `Notification` -> `Notification Received`
     - Pilih Aplikasi: **Aplikasi E-Wallet / Merchant / Bank Anda**
     - Text Content: `Matches (Berisi)` -> isi kata kunci seperti `berhasil`, `menerima`, `Rp`, atau kosongkan agar menangkap semua notifikasi dari aplikasi tersebut.
   - **Actions (Tindakan):**
...
4. Lakukan uji coba transfer QRIS. Bot akan otomatis mengenali nominal unik dan mengirimkan produk ke pembeli!
...
│   └── qris_generator.py      # Generator QRIS Dinamis Otomatis (EMVCo CRC-16)
│
