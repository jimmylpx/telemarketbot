...
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab-btn" onclick="switchTab('simulation')">🧪 Simulasi & Webhook QRIS</button>
        </div>
        ...
        <!-- Tab 5: Simulasi & Webhook QRIS -->
        <div id="tab-simulation" class="tab-pane">
            <div class="card">
                <div class="card-header">
                    <div class="card-title">🤖 Status Gateway & Endpoint Webhook QRIS</div>
                ...
                <p style="margin-bottom: 16px; color: #cbd5e1;">Public Webhook Endpoint: <code class="file-path" style="font-size: 13px;">{config.PUBLIC_URL}{config.WEBHOOK_PATH}</code></p>
                <p style="color: #94a3b8; font-size: 13px;">Halaman publik webhook sengaja disetel kosong/blank demi keamanan. Anda dapat melakukan simulasi notifikasi pembayaran (GoPay, DANA, Bank, dsb) langsung dari panel ini.</p>
            </div>
            ...
            <div class="card">
                ...
                <p style="color: #94a3b8; font-size: 14px; margin-bottom: 16px;">
                    Kirim simulasi notifikasi pembayaran/QRIS masuk langsung ke endpoint <code>{config.WEBHOOK_PATH}</code> untuk menguji auto-verifikasi dan pengiriman pesanan otomatis.
                </p>
                <form id="simForm" onsubmit="sendSimulation(event)">
                    <div class="form-group">
                        <label>Teks Notifikasi Pembayaran (GoPay, DANA, Bank, dsb):</label>
                        <input type="text" id="simNotifText" value="Pembayaran QRIS Rp 9.000 berhasil" required />
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        async function sendSimulation(e) {{
            ...
            resultDiv.innerHTML = '<span style="color: #38bdf8;">⏳ Mengirim permintaan webhook ke {config.WEBHOOK_PATH}...</span>';
            try {{
                const res = await fetch("{config.WEBHOOK_PATH}", {{
                    method: "POST",
